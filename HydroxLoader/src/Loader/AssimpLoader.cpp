...
namespace he
{
  namespace loader
  {
    ...
    sg::Scene* AssimpLoader::loadDefaultSceneGraph()
    {
      ...
      sg::GeoNode *geoNode = new sg::GeoNode(m_eventManager, m_modelManager->addObject(renderer::Mesh(positions, GL_TRIANGLES, indices)), materialLoader.getDefaultResource(), false, false, std::string("defaultCubeMesh"), sceneRootNode);
    ...
    sg::TreeNode* AssimpLoader::createSceneNodes(const aiScene *scene, const aiNode *node, std::vector<util::ResourceHandle>& meshes, sg::GroupNode *parentNode, sg::TreeNode *nextSibling)
    {
      ...
      for(unsigned int i = 0; i < node->mNumMeshes; i++)//all meshes are children of the current transformation node
      {
        ...
        {
          geoNode = new sg::AnimatedGeoNode(m_inverseBindPoseTable[meshIndex], m_eventManager, meshes[meshIndex], m_defaultMaterial, false, false, std::string(node->mName.C_Str()) + std::string("_Mesh") + stream.str(), parentNode, nextSibling);
          m_skinnedMeshTable[dynamic_cast<sg::AnimatedGeoNode*>(geoNode)] = m_boneNameTable[meshIndex];
        }
        else
        {
          geoNode = new sg::GeoNode(m_eventManager, meshes[meshIndex], m_defaultMaterial, false, false, std::string(node->mName.C_Str()) + std::string("_Mesh") + stream.str(), parentNode, nextSibling);
        }

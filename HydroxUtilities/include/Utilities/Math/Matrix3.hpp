...
namespace he
{
  namespace util
  {
    ...
    template<typename Type> std::ostream& operator<<(std::ostream& stream, const Matrix<Type, 3>& matrix)
    {
    ...
    template<typename Type> std::istream& operator>>(std::istream& stream, Matrix<Type, 3>& matrix)
    {
      for(unsigned int i = 0; i < 3; i++)
      {
        stream >> matrix[i];

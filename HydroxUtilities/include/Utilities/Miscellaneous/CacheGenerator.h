...
namespace he
{
  namespace util
  {
    ...
    public:
      ...
      CacheGenerator() : m_epsilon(0.00001f)
      {
      }
      //OCTREE GENERATION SHOULD BE REPLACED BY UNIFIED TREE MODEL!!!
      void generateCaches(std::vector<Cache>& outCaches, std::vector<vec2ui>& outTriangleCacheIndices, float errorRate, unsigned int subdivision, float maxAngle, const std::vector<vec3f>& positions, std::vector<unsigned int>& indices = std::vector<unsigned int>());

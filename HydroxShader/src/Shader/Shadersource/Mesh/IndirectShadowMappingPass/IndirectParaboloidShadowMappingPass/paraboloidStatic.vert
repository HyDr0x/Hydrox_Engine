...
void main()
{
	vsout_pos3D = trfMatrix[uint(gl_InstanceID + gl_BaseInstanceARB)] * vec4(in_Pos.xyz, 1.0);
	//vec4 projPos = reflectiveShadowLight[lightIndex].lightViewProj * vec4(vsout_pos3D.xyz, 1.0);
	//projPos.xyz /= projPos.w;
	//vec3 transformedPosition = projPos.xyz;
	
	vec3 transformedPosition = (reflectiveShadowLight[lightIndex].lightView * vec4(vsout_pos3D.xyz, 1.0)).xyz;

...
namespace he
{
  namespace db
  {
    ...
    void Mesh::generateCaches(float errorRate, float maxDistance, float maxAngle)
    {
      ...
      unsigned int newVertexNumber = cacheIndicesGenerator.getNewVertexNumber();

      if(newVertexNumber != 0)
      {
        ...
        copyDataIntoGeometryBuffer(MODEL_POSITION, m_vertexCount, newVertexNumber, reinterpret_cast<const GLubyte*>(&positions[0]));

        for(unsigned int j = 0; j < textureCoordinates.size(); j++)
        {
          if((m_vertexDeclaration & util::Flags<VertexElements>::convertToFlag(MODEL_TEXTURE0 + j)).toInt())
          {
            copyDataIntoGeometryBuffer(MODEL_TEXTURE0 + j, m_vertexCount, newVertexNumber, reinterpret_cast<const GLubyte*>(&textureCoordinates[j][0]));
          }
        }
        copyDataIntoGeometryBuffer(MODEL_NORMAL, m_vertexCount, newVertexNumber, reinterpret_cast<const GLubyte*>(&normals[0]));

        if((m_vertexDeclaration & util::Flags<VertexElements>::convertToFlag(MODEL_BINORMAL)).toInt())
        {
          copyDataIntoGeometryBuffer(MODEL_BINORMAL, m_vertexCount, newVertexNumber, reinterpret_cast<const GLubyte*>(&binormals[0]));
        }

        if((m_vertexDeclaration & util::Flags<VertexElements>::convertToFlag(MODEL_BONE_WEIGHTS)).toInt())
        {
          copyDataIntoGeometryBuffer(MODEL_BONE_WEIGHTS, m_vertexCount, newVertexNumber, reinterpret_cast<const GLubyte*>(&boneWeights[0]));
        }

        if((m_vertexDeclaration & util::Flags<VertexElements>::convertToFlag(MODEL_BONE_INDICES)).toInt())
        {
          copyDataIntoGeometryBuffer(MODEL_BONE_INDICES, m_vertexCount, newVertexNumber, reinterpret_cast<const GLubyte*>(&boneIndices[0]));
        }

        if((m_vertexDeclaration & util::Flags<VertexElements>::convertToFlag(MODEL_COLOR)).toInt())
        {
          copyDataIntoGeometryBuffer(MODEL_COLOR, m_vertexCount, newVertexNumber, reinterpret_cast<const GLubyte*>(&vertexColors[0]));
        }
      }

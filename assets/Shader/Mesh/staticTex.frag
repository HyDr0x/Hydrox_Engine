...
void main()
{
	...
	for(uint i = 0; i < 6; i++)
	{
		atomicAdd(globalCachesIndices[frameBufferIndex + i].x, 10);
		atomicAdd(globalCachesIndices[frameBufferIndex + i].y, 10);
		atomicAdd(globalCachesIndices[frameBufferIndex + i].z, 10);
		atomicAdd(globalCachesIndices[frameBufferIndex + i].w, 10);
	}*/
	/*
	for(uint i = 0; i < 6; i++)
	{

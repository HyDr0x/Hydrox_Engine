...
void main()
{
	...
	for(uint i = 0; i < 6; i++)
	{
		atomicAdd(globalCachesIndices[frameBufferIndex + i].x, 100);
		atomicAdd(globalCachesIndices[frameBufferIndex + i].y, 100);
		atomicAdd(globalCachesIndices[frameBufferIndex + i].z, 100);
		atomicAdd(globalCachesIndices[frameBufferIndex + i].w, 100);
	}*/
	/*
	for(uint i = 0; i < 6; i++)
	{
